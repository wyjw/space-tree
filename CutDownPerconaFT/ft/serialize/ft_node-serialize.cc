...
// Return 0 on success, otherwise an error number.
{
    ...
    invariant(file_size >= 0);
    fprintf(stdout, "Expected size of %d and file_size of %d\n", expected_size, file_size);
...
static void read_ftnode_header_from_fd_into_rbuf_if_small_enough_cutdown(int fd, _BLOCKNUM blocknum,
                                                                 ...
                                                                 ftnode_fetch_extra *bfe) {
    DISKOFF offset, size;
    BLOCKNUM _bnum = { .b = blocknum.b };
    ft->blocktable.translate_blocknum_to_offset_size(_bnum, &offset, &size);
    DISKOFF read_size = roundup_to_multiple(512, MIN(read_header_heuristic_max, size));
    uint8_t *XMALLOC_N_ALIGNED(512, roundup_to_multiple(512, size), raw_block);
    rbuf_init(rb, raw_block, read_size);
...
setup_available_ftnode_partition_cutdown(struct _ftnode *node, int i) {
	...
	MSN m = { .msn = node->max_msn_applied_to_node_on_disk.msn };
        BLB_MAX_MSN_APPLIED(cast_from__ftnode(node),i) = (MSN)m;
#ifdef DEBUG
...
static void compare_ftnode_and__ftnode(FTNODE a, struct _ftnode *b) {
	printk("Comparison testing!\n");
	int i = 0;
	if (a->max_msn_applied_to_node_on_disk.msn != b->max_msn_applied_to_node_on_disk.msn)
		printk("Diff msn.\n");
	if (a->flags != b->flags)
		printk("Diff flags.\n");
	if (a->blocknum.b != b->blocknum.b)
		printk("Diff bnum.\n");
	...
	if (a->n_children != b->n_children)
		printk("Diff n child.\n");
	// check offsets
	if (offsetof(struct ftnode, bp) != offsetof(struct _ftnode, bp))
		printk("Offsets are off!\n");
	if (offsetof(struct ftnode, pivotkeys) != offsetof(struct _ftnode, pivotkeys))
		printk("Offsets are off!\n");
}
...
static int deserialize_ftnode_partition_cutdown(
    ...
    int r = 0;
    const char *fname = _toku_ftnode_get_cachefile_fname_in_env(node);
    uint32_t data_size;
    data_size = sb->uncompressed_size - 4; // checksum is 4 bytes at end
...
static int decompress_and_deserialize_worker_cutdown(struct rbuf curr_rbuf,
    ...
    int r = 0;
    tokutime_t t0 = toku_time_now();
    r = read_and_decompress_sub_block(&curr_rbuf, (struct sub_block*)&curr_sb);
    if (r != 0) {
        const char *fname = _toku_ftnode_get_cachefile_fname_in_env(node);
        fprintf(stderr,
                "%s:%d:decompress_and_deserialize_worker - "
...
#endif
    // at this point, sb->uncompressed_ptr stores the serialized node partition
    r = deserialize_ftnode_partition_cutdown((struct sub_block*)&curr_sb, node, child, cmp);
    if (r != 0) {
        const char *fname = _toku_ftnode_get_cachefile_fname_in_env(node);
        fprintf(stderr,
                "%s:%d:decompress_and_deserialize_worker - "
...
int check_and_copy_compressed_sub_block_worker_cutdown(struct rbuf curr_rbuf,
                                                      ...
                                                      int child) {
    int r = 0;
    r = read_compressed_sub_block(&curr_rbuf, (struct sub_block *)&curr_sb);
    if (r != 0) {
        goto exit;
    }

    SUB_BLOCK bp_sb;
    bp_sb = BSB(cast_from__ftnode(node), child);
    bp_sb->compressed_size = curr_sb.compressed_size;
    bp_sb->uncompressed_size = curr_sb.uncompressed_size;
    bp_sb->compressed_ptr = toku_xmalloc(bp_sb->compressed_size);
    memcpy(
...
inline const char* toku_ftnode_get_cachefile_fname_in_env_cutdown(struct _ftnode *node) {
    if (node->ct_pair) {
...
int deserialize_ftnode_info_cutdown(struct _sub_block *sb, struct _ftnode *node) {
    int r = 0;
    dump_ftnode_cutdown(node);
    const char *fname = toku_ftnode_get_cachefile_fname_in_env_cutdown(node);
...
int _search_which_child(const toku::comparator &cmp, struct _ftnode *node, ft_search *search) {
	struct _dbt a;
	_init_dbt(&a);
...
void setup_ftnode_partitions_cutdown(struct _ftnode* node, ftnode_fetch_extra *bfe, bool data_in_memory) {
	...
	if (bfe->type == ftnode_fetch_subset)
	{
		lc = bfe->leftmost_child_wanted(cast_from__ftnode(node));
		rc = bfe->rightmost_child_wanted(cast_from__ftnode(node));
	}
...
static int deserialize_ftnode_from_rbuf_cutdown(struct _ftnode **ftnode,
    ...
    for (int i = 0; i < node->n_children; i++) {
        ...
        case PT_COMPRESSED:
            ...
            r = check_and_copy_compressed_sub_block_worker(curr_rbuf, *(struct sub_block *)&curr_sb, cast_from__ftnode(node), i);
                fprintf(
                    stderr,
                    "%s:%d:deserialize_ftnode_from_rbuf - "
                    "file[%s], blocknum[%ld], childnum[%d], "
...
static int deserialize_ftnode_from_fd_cutdown(int fd,
    ...
    int r = deserialize_ftnode_from_rbuf_cutdown(
...
int toku_deserialize_ftnode_from_cutdown(int fd,
    ...
    if (!bfe->read_all_partitions) {
        read_ftnode_header_from_fd_into_rbuf_if_small_enough_cutdown(
            fd, blocknum, bfe->ft, &rb, bfe);
        r = deserialize_ftnode_header_from_rbuf_if_small_enough_cutdown(
            ftnode, ndd, blocknum, fullhash, bfe, &rb, fd);
    } else {
        // force us to do it the old way
        r = -1;
    ...
    if (r != 0) {
        // Something went wrong, go back to doing it the old way.
        r = deserialize_ftnode_from_fd_cutdown(
            fd, blocknum, fullhash, ftnode, ndd, bfe, nullptr);
    }
